// Battleship.cpp : Este archivo contiene la función "main". La ejecución del programa comienza y termina ahí.
//
...
int main() {
    ...
    CROW_ROUTE(app, "/guardar_users")
        .methods("POST"_method)([](const crow::request &req){
          auto x = crow::json::load(req.body);
           if (!x) return crow::response(400);
          string nombre = x["usuario"].s();
          string contra = x["contrasena"].s();
          int monedas = x["usuario"].i();
          int edad = x["usuario"].i();
          if (!buscarNodo(nombre))
          {
              insertarmasivo(nombre, contra, monedas, edad);
          }
          return crow::response(200);
            });

    // Puto si muevo esto xd
    app.port(5000).run();
}
